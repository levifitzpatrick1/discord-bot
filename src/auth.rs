...
pub async fn get_oath_token() -> Result<String, Box<dyn std::error::Error>> {
    ...
    let response = client.post("https://us.battle.net/oauth/token").basic_auth(client_id, Some(client_secret)).form(&[("grant_type", "client_credentials")]).send().await?;
...
pub fn get_global_token() -> String {
    ...
    token.clone().unwrap_or_else(|| "Token not set".to_string())
}